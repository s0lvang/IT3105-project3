...
class BasicClientActorAbs(ABC):
    ...
    def connect_to_server(self):
        ...
        print(
            "Attempting to connect to server using ip-address: "
            + self.IP_address
            + ":33000..."
        )
        ...
        while True:
            ...
            # We received a game end message
                ...
                self.handle_game_over(
                    ast.literal_eval(winner), ast.literal_eval(end_state)
                )
            ...
            # We received an illegal action message
                ...
                self.handle_illegal_action(
                    ast.literal_eval(state), ast.literal_eval(illegal_action)
                )
                break
