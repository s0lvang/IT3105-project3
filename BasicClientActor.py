...
from main import load_policies
...
class BasicClientActor(BasicClientActorAbs):
    def __init__(self, IP_address=None, verbose=True):
        ...
        policies = load_policies("1r3e-28T14:43")
        self.policy = policies[max(policies)]
    ...
    def handle_get_action(self, state):
        ...
        print(state[0])
        print(state[1:])
        game = Game(list(state[1:]), state[0])
        prediction = self.policy.predict(list(state[1:]), state[0])
        next_move = game.get_action_from_network_output(prediction)
